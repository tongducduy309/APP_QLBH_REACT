...
function buildQuotationContent(data: DataPdf, opt: PrintOptions): Content[] {
  ...
  opt.pageOrientation =
    opt.paperSize === "A5" ? "landscape" : opt.pageOrientation;
  ...
  const hasShip = Number(data.shippingFee || 0) > 0;
    ...
    ...(hasSubtotal
      ? [
        [
          ...
          {
            text: formatMoney(data.subtotal || 0),
            alignment: "left",
            style: "s12",
          },
        ],
    ...
    ...(hasTax
      ? [
        [
          ...
          {
            text: formatMoney(data.taxAmount || 0),
            alignment: "left",
            style: "s12",
          },
        ],
    ...
    ...(hasShip
      ? [
        [
          ...
          {
            text: formatMoney(data.shippingFee || 0),
            alignment: "left",
            style: "s12",
          },
        ],
  ...
  const hasSummary = leftBody.length > 0 || rightBody.length > 0;
  ...
  const maxRows = Math.max(
    leftTable.table.body.length,
    rightTable.table.body.length
  );

  leftTable = padTable(leftTable, maxRows);
  rightTable = padTable(rightTable, maxRows);

  const summary: Content | null = hasSummary
    ? {
      table: {
        widths: ["*", "*"],
  ...
  const headerColumns: any[] = [
      ...
      : { width: 80, text: "" },
    {
      ...
      stack: [
        {
          text: "CÔNG TY TNHH MTV DV TÔN THÉP TÂM ĐỨC CƯỜNG",
          style: "companyName",
        },
        {
          text: "Địa chỉ: 220 Lũy Bán Bích - P.Tân Phú - TP.Hồ Chí Minh",
          style: "companyInfo",
        },
        {
          text: "Chi nhánh: 413 Nguyễn Văn Tạo - Xã Hiệp Phước - TP.Hồ Chí Minh",
          style: "companyInfo",
        },
        {
          text: "SĐT: 0918.279.361 - 0933.770.378   MST: 0305971408",
          style: "companyInfo",
        },
        {
          text: "Ngân hàng: 060128011741 (SACOMBANK)",
          style: "companyInfo",
        },
      ],
      alignment: "center",
  ...
  return [
    ...
    ...(hasTotal
      ? [
        {
          text: `\nBằng chữ: ${numberToVietnameseWords(data.total || 0)}`,
          italics: true,
          style: "s12",
        },
      ]
          ...
          [
            ...
            { text: "" },
            {
              stack: [
                ...
                { text: "\nGiám đốc", style: "s12", alignment: "center" },
                {
                  text: "\n\n\n\nTỐNG ĐỨC CƯỜNG",
                  style: "s12",
                  alignment: "center",
...
function buildQuotationDocDefinition(
  ...
  for (let i = 0; i < copies; i++) {
    content.push(...buildQuotationContent(data, opts));
    if (i < copies - 1) {
  ...
  return {
    ...
    watermark: {
      ...
      opacity: 0.08,    // nhẹ hơn (đỡ che nội dung)
      bold: true,
      italics: false,
      fontSize: 70,
      angle: -30,
    ...
    info: {
      ...
      keywords: "bao gia, quotation, ton thep",
      creator: "QLBH",
      producer: "QLBH App",
