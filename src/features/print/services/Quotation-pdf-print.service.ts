  ...
  return [
    {
      columns: [
        { image: 'logo', width: 80 },
        {
          width: '*',
          stack: [
            { text: 'CÔNG TY TNHH MTV DV TÔN THÉP TÂM ĐỨC CƯỜNG', style: 'companyName' },
            { text: 'Địa chỉ: 220 Lũy Bán Bích - P.Tân Phú - TP.Hồ Chí Minh', style: 'companyInfo' },
            { text: 'Chi nhánh: 413 Nguyễn Văn Tạo - Xã Hiệp Phước - TP.Hồ Chí Minh', style: 'companyInfo' },
            { text: 'SĐT: 0918.279.361 - 0933.770.378   MST: 0305971408', style: 'companyInfo' },
            { text: 'Ngân hàng: 060128011741 (SACOMBANK)', style: 'companyInfo' }
          ...
          [
            ...
            { text: "" },
            {
              stack: [
                ...
                { text: "\n\n\n\nTỐNG ĐỨC CƯỜNG", style: "s12", alignment: "center", bold: true },
              ],
            },
          ],
        ],
...
export function downloadQuotation(data: OrderRes, opts: PrintOptions = {}) {
  const name = formatDateToDDMMYYYY(data.createdAt??new Date().toISOString())
  const dd = buildQuotationDocDefinition(data, opts);
